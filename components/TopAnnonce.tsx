...
const TopAnnonces = () => {
  ...
  useEffect(() => {
    const fetchAnnonces = async () => {
      try {
        // Fonction de nettoyage des titres (ex: &#8211;)
        ...
        // On charge seulement les 3 dernières
  ...
  // --- COMPOSANT VISUEL "CAPSULE" (Même que AnnoncesRecente) ---
  const AnnonceVisual = ({ annonce }: { annonce: any }) => {
    
    // CAS 1 : Transport International (Capsule)
  ...
  if (loading) return <div className="py-20 text-center animate-pulse text-gray-500">Chargement des offres...</div>;
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center mb-12 gap-4">
          <div className="text-center sm:text-left">
            <h2 className="text-4xl font-black text-[#104C9E] tracking-tight">À la une</h2>
            <p className="text-gray-600 text-lg mt-1">Les 3 offres les plus récentes</p>
          </div>
          
          <Link href="/annonces" className="group flex items-center gap-2 px-5 py-2.5 bg-white border border-gray-200 rounded-full hover:border-[#104C9E] hover:text-[#104C9E] transition-all shadow-sm text-gray-700 font-semibold">
            <span>Voir tout</span>
                ...
                <div className="p-6 flex-grow flex flex-col">
                  ...
                  <div className="pt-5 border-t border-gray-100 flex justify-between items-center mt-auto">
                    ...
                    <div className="flex space-x-2">
                        <button className="w-9 h-9 rounded-full bg-gray-50 hover:bg-red-50 flex items-center justify-center transition-colors group/btn" onClick={(e) => e.stopPropagation()}>
                            <Heart className="w-4 h-4 text-gray-400 group-hover/btn:text-red-500 group-hover/btn:fill-red-500 transition-all" />
                        </button>
                        <button className="w-9 h-9 rounded-full bg-gray-50 hover:bg-[#104C9E] flex items-center justify-center transition-colors group/btn" onClick={(e) => e.stopPropagation()}>
